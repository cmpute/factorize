...
PATH = Dict({ # path to various factorization utilities
    "factor": "/usr/bin/factor", # GNU coreutils/factor
    "factorize": "~/factorize/target/release/factorize", # Rust factorize
...
def test_bin(path, nums):
    tstart = time()
    check_call([expanduser(path)] + [str(n) for n in nums], stdout=DEVNULL)
    tend = time()
